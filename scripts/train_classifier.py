...
# Use the bilingual tokenizer checkpoint selected in Lab 1
CHECKPOINT = "xlm-roberta-base"

# Keep the sequence length above the measured Lab 1 p95 values
MAX_LENGTH = 128
...
def main():
   ...
   # Load the leakage-safe train, validation, and test splits
dataset = build_topic_dataset()

# Build deterministic label mappings from the training topics
topic_names = sorted(set(dataset["train"]["topic"]))
...
# Tokenize all splits and remove columns not required by the model
...
# Load XLM-R with a classification head for Bayan topics
...
# Configure GPU training and model checkpointing
...
# Evaluate the best checkpoint on validation and frozen test data
...
# Save the model, tokenizer, state, and measured metrics
trainer.save_model(str(output_dir))
tokenizer.save_pretrained(str(output_dir))
trainer.save_state()

train_metrics = dict(train_result.metrics)
train_metrics["wall_clock_seconds"] = training_time
